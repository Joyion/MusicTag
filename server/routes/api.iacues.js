...
const composers = require('../models/ia_composer.model');
const Publisher = require("../models/ia_publisher.model");
const path = require("path");
const mp3path = path.join(__dirname, "..", "..", "public", "dist","ia", "mp3");
const wavPath = path.join(__dirname, "..", "..", "public", "dist","ia", "wav")
...
router.get("/getIACues", (req, res) => {
    ...
    iaCue.countDocuments({ status: data.status }, function (err, count) {
        if (err) {
            ...
            iaCue.find({ status: data.status })
            .populate({ path: 'composers.composer', model: "IAComposer" })
            .populate({ path: "publishers.publisher", model: "IAPublisher" })
...
router.put("/copyCue", (req, res) => {
    ...
    iaCue.findOne({ fileName: mv }, (err, cue) => {
        if (err) {
            ...
            iaCue.findByIdAndUpdate(req.body.id, {
                ...selectCue,
                artist: cue.artist,
...
router.put("/updateCue", function (req, res) {
    ...
    if (name == "addComposer") {
        ...
        iaCue.findByIdAndUpdate(id, { ...update }, { new: true }, (err, cue) => {
            if (err) {
                const error = { error: true, message: "UPDATE FAILED" }
                res.status(400).json(JSON.stringify(error))
            }
